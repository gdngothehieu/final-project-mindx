
 const cities =  [
    {
        city: "Da Nang",
        country: "Vietnam"
    },
    {
        city: "Hồ Chí Minh",
        country: "Vietnam"
    },
    {
        city: "Hà Nội",
        country: "Vietnam"
    },
]

  const mindXHotel =  [
    {   
        title: "MindX",
        address: "1 Trường Chinh",
        location: "Đà Nẵng",
        availability: "2024-2-10" ,
        facilities: ["Máy Lạnh","wifi","Bàn ủi"],
        review: 5,
        price: 600000
    },
    {
        
        title: "MindX Hotel (Quang Trung)",
        address: "1 Quang Trung",
        location: "Hồ Chí Minh",
        availability: "2024-2-10" ,
        price: 600000,
        review: 5,

        facilities: ["Máy Lạnh","wifi","Bàn ủi"],
    },
    {
        title: "MindX Hotel (Song Hành)",
        address: "2 Song Hành",
        location: "Hồ Chí Minh",
        availability: "2024-2-10" ,
        facilities: ["Máy Lạnh","wifi","Bàn ủi"],
        price: 600000,
        review: 5,

    },
    
    {
        title: "MindX Hotel",
        address: "3 P.Lương Yên",
        location: "Huế",
        ava: "2024-2-10" ,
        availability: "2024-2-10" ,
        facilities: ["Máy Lạnh","wifi","Bàn ủi"],
        price: 600000,
        review: 5,

    },
    {
        title: "MindX Hotel (Thành Công)",
        address: "Tầng 6, toà C, 22 Thành Công Hanoi Hanoi City, 22 P. Thành Công",
        location: "Hà Nội",
        availability: "2024-2-10" ,
        price: 600000,
        review: 5,

        facilities: ["Máy Lạnh","wifi","Bàn ủi"],
    },
    {
        title: "MindX Hotel (Hoàng Đạo Thúy)",
        address: "29 T1 Hoàng Đạo Thúy",
        location: "Hà Nội",
        availability: "2024-2-10" ,
        price: 600000,
        facilities: ["Máy Lạnh","wifi","Bàn ủi"],
        review: 5,

    },

]

export  {mindXHotel,cities};